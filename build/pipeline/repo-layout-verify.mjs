...
// 外层可以固定比 Desktop gitlink 更新的 Harness；只忽略嵌套工作树状态，
// 下方仍单独核对 Harness HEAD 和源码洁净性，不忽略 Desktop 自身修改。
if (requireClean && git(desktopPath, 'status', '--porcelain', '--ignore-submodules=all') !== '') {
...
if (manifest.plugins.some(plugin => plugin.enabledByDefault === true)
  && !existsSync(resolve(root, 'build', 'pipeline', 'product.yarn.lock'))) {
  fail('enabled product plugins require build/pipeline/product.yarn.lock; run product:refresh-lock')
}

// 市场名册、产品清单和生成快照各写一份内置插件版本，三者曾经分叉，
// 发版校验时一并核对。
execFileSync(process.execPath, [resolve(root, 'build', 'modules', 'market', 'market-catalog-verify.mjs')], {
  cwd: root,
  stdio: 'inherit',
})
