...
管理登录改为服务端 D1 会话，浏览器使用 Secure / HttpOnly / SameSite=Strict、仅当前主机的 Cookie。
默认有效期 7 天，刷新和重新打开页面自动恢复；退出删除服务端会话并清除 Cookie，更换管理密码自动使旧会话失效。
浏览器不保存管理密码，数据库只保存随机会话令牌的摘要。Cookie 管理写请求和登录/退出均校验同源 Origin。
此约定替代下文“刷新需要重新登录”的临时实现；原 Bearer 管理接口兼容维护脚本。

## 最新调整：恢复管理登录与完整 Key 展示（2026-09-08）

- 取消免登录模式；旧 `MARKET_ADMIN_MODE=open` 不再生效。所有管理接口始终校验服务端 `MARKET_ADMIN_TOKEN`。
- 未登录只展示登录页。管理凭证仅保留在当前页面内存，刷新需要重新登录，退出清空管理数据及 Key；默认凭证通过部署端设置，不写入前端或仓库。
- 新录入的独立 Key 以 AES-GCM 加密保存在 `market_plugin_key_values`，独立密钥 `MARKET_KEY_ENCRYPTION_SECRET` 只保存在 Cloudflare secret；授权仍按原 HMAC 指纹匹配，不更换现有 HMAC secret。
- 完整 Key 仅通过鉴权后的 `GET /api/admin/plugin-key-values?id=...` 按插件返回，响应不可缓存；常规目录及管理列表不携带完整 Key。
- 旧记录没有密文，不能还原。重新录入同一 Key 可补全显示并去重，原有授权不中断。移除授权同时删除该插件对应密文。
- 管理员凭证和加密密钥是不同秘密；更换管理员凭证不影响已保存的 Key。不要直接轮换加密密钥，否则旧密文不能解密。
- 以下免登录、仅存指纹等段落保留为历史，以上述最新调整为准。
