...
**谁能看到和下载一个插件，只由后台的访问范围决定；Registry 只负责存包。**

- 「公开」的条目对所有人可见，任何人都能通过 `/registry/` 代理拿到元数据和 tarball——即使这个包在 Registry 里放在需要登录的作用域下，市场代理也会用服务账号取包后分发。把私有作用域的包设为公开是一个真实的分发决定，后台会要求明确确认。
- 「受限」的条目先校验市场的组织/Key 授权，再用服务账号取包；没有授权就看不到、装不上。
- Registry 自身的 `access` 规则只管有人**绕过市场**直接把 npm 客户端指到这个源时能读到什么，与市场里插件的公开/受限无关，两边不需要同步。
...
MARKET_PRIVATE_REGISTRY_AUTH_SCHEME=basic
