...
export async function resolveNpmVersions(items) {
  ...
  await Promise.all(Array.from({length: Math.min(6, names.length)}, async () => {
    while (next < names.length) {
      ...
      versions.set(name, await latestVersion(name, ''))
    }
  }))
...
export async function liveCatalog(items) {
  return (await resolveNpmVersions(items))
