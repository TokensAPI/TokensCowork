...
test('puts _?= last so NSIS parses it as the in-place directory', () => {
  const args = buildUninstallArguments('C:\\Apps\\TokensCowork', { deleteAppData: true })
  assert.equal(args.at(-1), `_?=${resolve('C:\\Apps\\TokensCowork')}`)
  assert.deepEqual(args.slice(0, -1), ['/S', '--delete-app-data'])
})

test('leaves the _?= path unquoted because NSIS takes quotes literally', () => {
  const args = buildUninstallArguments('C:\\Program Files\\TokensCowork')
  const flag = args.at(-1)
  assert.ok(!flag.includes('"'), `path must stay unquoted, got ${flag}`)
  assert.equal(flag, `_?=${resolve('C:\\Program Files\\TokensCowork')}`)
})

test('keeps user data by default because deletion is opt-in', () => {
  const args = buildUninstallArguments('C:\\Apps\\TokensCowork')
  assert.deepEqual(args, ['/S', `_?=${resolve('C:\\Apps\\TokensCowork')}`])
})

test('keeps user data when the caller explicitly declines deletion', () => {
  const args = buildUninstallArguments('C:\\Apps\\TokensCowork', { deleteAppData: false })
...
test('never emits /KEEP_APP_DATA, which the uninstaller does not parse', () => {
  ...
  for (const options of [undefined, { deleteAppData: true }, { deleteAppData: false }]) {
    const args = buildUninstallArguments('C:\\Apps\\TokensCowork', options)
...
test('extracts the quoted uninstaller path and ignores registry arguments', () => {
  assert.equal(
    parseUninstallExecutable('"C:\\Users\\tester\\AppData\\Local\\Programs\\TokensCowork\\Uninstall TokensCowork.exe" /currentuser'),
    'C:\\Users\\tester\\AppData\\Local\\Programs\\TokensCowork\\Uninstall TokensCowork.exe',
  )
})
...
test('derives the install directory when electron-builder omits InstallLocation', () => {
  assert.deepEqual(
    normalizeInstallInfo({
      InstallLocation: '',
      UninstallString: '"C:\\Users\\tester\\AppData\\Local\\Programs\\TokensCowork\\Uninstall TokensCowork.exe" /currentuser',
    }),
    {
      installLocation: resolve('C:\\Users\\tester\\AppData\\Local\\Programs\\TokensCowork'),
      uninstallString: 'C:\\Users\\tester\\AppData\\Local\\Programs\\TokensCowork\\Uninstall TokensCowork.exe',
    },
  )
})
...
test('prefers a populated InstallLocation from the registry', () => {
  assert.equal(
    normalizeInstallInfo({
      InstallLocation: 'C:\\Apps\\TokensCowork',
      UninstallString: '"C:\\Elsewhere\\Uninstall TokensCowork.exe" /currentuser',
    }).installLocation,
    resolve('C:\\Apps\\TokensCowork'),
  )
})
...
test('accepts the uninstaller left behind by in-place execution', () => {
  assert.equal(isUninstallComplete(['Uninstall TokensCowork.exe'], true), true)
...
test('rejects a directory where the application files survived', () => {
  ...
  assert.equal(isUninstallComplete(['TokensCowork.exe', 'resources'], true), false)
  assert.equal(isUninstallComplete(['Uninstall TokensCowork.exe', 'resources'], true), false)
})

test('rejects a directory holding a lookalike that is not the uninstaller', () => {
  assert.equal(isUninstallComplete(['Uninstall TokensCowork.exe.bak'], true), false)
...
const upgradeGuard = readFileSync(
  resolve(import.meta.dirname, '..', 'build', 'modules', 'platform', 'assets', 'windows', 'installer', 'upgrade-guard.nsh'),
  'utf8',
)
